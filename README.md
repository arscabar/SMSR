...
![SMSR 샘플 작업 그래프](docs/images/sample-workflow-graph.svg)

완료·검증·진행 중 노드와 목표/그래프 토큰 분류를 함께 표시한 대시보드 예시입니다.
...
- 목표 토큰은 Codex 세션 누적값에서 목표 시작 기준값을 뺀 실제 증분이다. 그래프 토큰은 SMSR 추적 요청·응답 크기로 계산한 추정치이며 로컬 JSONL 파일 쓰기는 모델 토큰을 소비하지 않는다.
