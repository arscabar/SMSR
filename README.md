...
`작업 펫`에는 10MB 이하 PNG·JPG·BMP 한 장을 등록할 수 있습니다. SMSR은 원본을 직접 참조하지 않고 로컬 데이터 폴더에 복사한 뒤, 작업 현황에서 선택한 그래프의 진행률과 상태를 펫 창에 표시합니다. 트레이 메뉴에서 펫을 표시하거나 숨길 수 있으며, 차단·실패·진행·검증·완료 상태에 따라 움직임이 달라집니다.
...
2. 펫 크기·투명도·움직임 감소와 스프라이트 애니메이션을 사용성 확인 후 추가한다.
