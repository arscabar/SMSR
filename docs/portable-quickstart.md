...
SMSR은 현재 실행 파일 경로를 Windows 로그인 자동 시작, `~/.codex/hooks.json`, `~/.codex/config.toml`의 로컬 stdio MCP 명령에 병합합니다. OAuth 인증이나 별도 Codex CLI는 필요하지 않으며 다른 저장소에서도 같은 사용자 Codex 환경이면 별도 프로젝트 설정 없이 적용됩니다.

일반 작업은 SMSR에 기록하지 않습니다. `이 작업을 SMSR 그래프로 추적해줘`처럼 명시적으로 요청하면 해당 작업이 끝날 때까지만 추적합니다. 이전 그래프는 목록을 요청한 뒤 workflow ID를 선택해 이어갈 수 있습니다.
