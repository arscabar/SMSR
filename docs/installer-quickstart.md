...
SMSR은 설치된 실행 파일을 Codex의 로컬 stdio MCP로 자동 등록하므로 OAuth 인증 화면이 없습니다. PC를 종료하거나 재부팅해도 Windows 자동 시작과 Codex 설정으로 재연결됩니다. SMSR 본체가 꺼져 있어도 다음 Codex 시작 시 브리지가 본체와 서버를 백그라운드로 자동 실행하므로 사용자가 에이전트에게 인증이나 실행을 따로 요청할 필요가 없습니다.
...
작업 현황의 캘린더는 모든 Codex 프로젝트의 그래프를 수신 날짜별로 함께 보여 줍니다. 날짜를 고르면 그날의 최신 작업이 열리고 같은 날짜의 다른 작업도 옆 목록에서 선택할 수 있습니다. 다른 프로젝트의 새 이벤트가 들어와도 현재 선택은 유지됩니다.

`기록 관리`에서 선택 작업·현재 프로젝트·전체 이력을 확인 후 삭제할 수 있습니다. SQLite, 활동 JSONL과 자동추적 연결이 함께 정리되며 내보낸 ZIP/HTML과 앱 설정은 유지됩니다.

설치 Wizard는 SMSR 브랜드 환영·완료 화면을 표시하며 Windows의 라이트·다크 모드에 맞춰 자동 전환됩니다. `/VERYSILENT` 무인 설치에서는 Wizard 화면을 표시하지 않습니다.

## 시스템 트레이

트레이 아이콘의 우클릭 메뉴에서 현재 서버·Codex 상태, SMSR 열기, 현재 대시보드 열기, 서버 시작·중지, 설정 열기와 완전 종료를 사용할 수 있습니다. 연결됨은 녹색, 연결 대기는 주황색, 서버 중지는 빨간색으로 구분합니다. 선택된 워크플로우가 없으면 대시보드 메뉴는 비활성화되고, 서버 상태에 따라 시작 또는 중지만 활성화됩니다. 트레이 아이콘을 더블클릭하거나 시작 메뉴에서 SMSR을 다시 실행하면 숨겨진 기존 창을 복원합니다.
...
```powershell
SMSR-Setup-1.2.0.0-win-x64.exe /VERYSILENT /SUPPRESSMSGBOXES /NORESTART
```
