...
같은 Windows 사용자에서는 PC를 종료하거나 재부팅해도 OAuth 승인이 유지됩니다. 재부팅 직후 SMSR 화면에 `OAuth 인증 유지됨`이 보이면 다시 인증하지 말고 Codex에서 SMSR 도구가 처음 호출될 때까지 기다리면 됩니다. 다른 PC나 다른 Windows 사용자에서는 보안상 최초 승인이 한 번 필요합니다.
