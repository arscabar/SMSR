...
public static class PlanningPromptSettings
{
    ...
    public const string Default = """
        ...
        서로 독립적인 작업은 병렬 진행 가능 여부를 표시하세요. 포괄적인 작업 안에 독립적으로 진행·중단·실패·완료를 판단할 수 있는 두 개 이상의 실행 결과가 있으면, 그 작업을 부모로 두고 A1, A2 등의 하위 작업으로 분리하세요. 같은 원인으로 3회 실패하면 재시도를 중단하고 실패 원인, 시도 내역과 필요한 결정을 보고하세요. 모든 필수 작업과 검증이 성공했을 때만 완료로 판정하세요. 계획 초안을 사용자에게 보여주고 수정 의견 또는 진행 승인을 받은 뒤 작업을 시작하세요. 사용자가 계획 검토 생략을 명시하면 바로 진행하세요. SMSR 그래프는 사용자가 명시적으로 요청했거나 설정에서 허용한 복잡한 프로젝트 변경에만 적용하고, 계산·검색·질문·작은 단일 수정에는 만들지 마세요.
        """;
