...
internal static class CodexAutoTrackingContext
{
    ...
    internal static string CreateOutput(string input)
    {
        ...
        var context = $"""
            SMSR graph tracking is opt-in and ordinary work must create no SMSR records. Use the smsr tools only when the user explicitly asks to track or visualize work as a graph, flow, dashboard, or SMSR workflow, or explicitly asks to resume a previous graph. This hook's current Codex task/session ID is exactly '{workflowId}'. For a new graph use projectId '{projectId}' and that exact current task/session ID as the base workflowId. Ignore source_thread_id, delegation source or parent task IDs, wrapper IDs, and IDs from prior tasks. To resume, call list_workflows for the project, load the selected workflow with get_plan and get_state, and keep its workflowId. Continue updates until SUCCESS, FAILED, or BLOCKED, send the final event, then stop heartbeats and never attach unrelated requests. Never send prompt text, secrets, raw command contents, or every tool call.
            """;
