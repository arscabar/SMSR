...
internal static class CodexAutoTrackingContext
{
    ...
    internal static string CreateOutput(string input, AppSettings? settings = null)
    {
        ...
        var context = $"""
            Before the final reply, call get_state and record every remaining PENDING, IN_PROGRESS, VALIDATING, or RETRYING node as SUCCESS, FAILED, BLOCKED, or CANCELLED according to its actual result. Use CANCELLED when work stops unfinished or moves to another request; never leave active nodes behind after work ends.
            SMSR separates daily activity from graphs. Never record calculations, quick lookups, Q&A, read-only inspection, status checks, navigation, or commands that do not change project files. After any request that actually changes project files, call record_daily_activity exactly once before the final reply with a unique activityId for that request, a concise title, result summary, changed file paths, validation results, and artifacts; use the current task/session ID '{workflowId}' as taskId and project folder '{projectId}' as projectId. Reuse the activityId only to correct that same card. Link workflowId only when a graph was used. Never store prompt text or raw commands.{automaticGraph} An explicit user request to track or visualize a graph always overrides the complexity threshold. Ambiguous or single-location small edits do not create graphs and use only record_daily_activity. A graph scope ends when every planned node is terminal; later work starts no graph unless that request independently qualifies or the user explicitly requests one. For a new graph omit workflowId in the first save_plan call and reuse the returned readable ID only for that scope. Before save_plan, inspect every proposed leaf: if it combines two or more independently trackable child tasks, outcomes, handoffs, or dependency stages, replace broad parent A with children A1, A2, and so on; the server assigns parentNodeId so its click opens the nested graph. Never leave a broad parent as a leaf when child states can show whether work is progressing or stopped. Do not create a nested graph from duration alone; use it only for independently trackable work. Update an active graph before related follow-up work, preserve completed nodes, and add new sibling/root nodes with dependsOn. Never reopen or add children to SUCCESS nodes. On the first node-start event and each related follow-up request added to an active graph, begin summary with '사용자 요청 요약:' followed by a one- or two-sentence paraphrase. Never copy the raw prompt, secrets, or personal data. Send record_event immediately on meaningful progress and heartbeat within 30 seconds only during quiet active work. Complete predecessors before successors; only independent nodes run in parallel. To resume, use list_workflows, get_plan and get_state. Never send secrets, prompt text, raw commands, tool inputs or outputs.{planning}
            """;
