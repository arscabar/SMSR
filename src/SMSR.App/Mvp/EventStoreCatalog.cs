...
public sealed partial class EventStore
{
    public Task<IReadOnlyList<string>> GetProjectIdsAsync(CancellationToken cancellationToken = default)
        => GetIdsAsync("SELECT project_id FROM events UNION SELECT project_id FROM plan_nodes ORDER BY project_id LIMIT 200;", null, cancellationToken);

    public Task<IReadOnlyList<string>> GetWorkflowIdsAsync(string projectId, CancellationToken cancellationToken = default)
        => GetIdsAsync("SELECT workflow_id FROM events WHERE project_id = $projectId UNION SELECT workflow_id FROM plan_nodes WHERE project_id = $projectId ORDER BY workflow_id LIMIT 200;", projectId, cancellationToken);
