...
public sealed partial class EventStore
{
    public Task<IReadOnlyList<string>> GetProjectIdsAsync(CancellationToken cancellationToken = default)
        => GetIdsAsync("SELECT project_id FROM events UNION SELECT project_id FROM plan_nodes UNION SELECT project_id FROM agent_heartbeats UNION SELECT project_id FROM daily_activities ORDER BY project_id LIMIT 200;", null, cancellationToken);
