...
internal sealed class McpBridgeToken(string dataPath)
{
    private static readonly byte[] Entropy = Encoding.UTF8.GetBytes("SMSR-mcp-bridge-v1");
    private readonly string _path = Path.Combine(dataPath, "mcp-bridge-token.bin");
    ...
    public bool Validate(string candidate)
    {
        ...
        return CryptographicOperations.FixedTimeEquals(
            Encoding.UTF8.GetBytes(Value), Encoding.UTF8.GetBytes(candidate));
    }
